...
const GalleryClassName3 = 'gallery3';//(9) Создаем переменную gallery(общий контейнер)
const GalleryDraggableClassName3 = 'gallery-draggable3';//(96) Создаем переменную gallery-draggable(перетаскиваемый элемент)
const GalleryLineClassName3 = 'gallery-line3';//(10) Создаем переменную gallery-line(контейнер для слайдов)
const GallerySlideClassName3 = 'gallery-slide3';//(11) Создаем переменную gallery-slide(слайд)
const GalleryDotsClassName3 = 'gallery-dots3';//(102) Создаем переменную gallery-dots(контейнер для точек)
const GalleryDotClassName3 = 'gallery-dot3';//(103) Создаем переменную gallery-dot(точки)
const GalleryDotActiveClassName3 = 'gallery-dot-active3';//(104) Создаем переменную gallery-dot-active(фстивная точка)
const GalleryNavClassName3 = 'gallery-nav3';//(105) Создаем переменную gallery-nav(контейнер для стрелок)
const GalleryNavLeftClassName3 = 'gallery-nav-left3';//(106) Создаем переменную gallery-nav-left(левая стрелка)
const GalleryNavRightClassName3 = 'gallery-nav-right3';//(107) Создаем переменную gallery-nav-right(правая стрелка)
const GallerySlideActiveClassName3 = 'gallery-slide-active3';//Актив для текущего слайда////////////////////////////////////////




class Gallery3 {//(1) Создаём класс Gallery 
	constructor(element, options = {}) {//(2) Объявляем конструктор (передаём ему Gallery(element)) добавляем опции
		this.containerNode = element;//(3) Объявляем this.containerNode куда сохраняем елемент
		this.size = element.childElementCount;//(4) Задаем размер элементу(element) через количество дочерних элементов (childElementCount)		
		this.currentSlide = 2;//(5) Первый слайд нулевой
		this.currentSlideWasChanged = false;//(82)текущийСлайдБылИзменен(currentSlideWasChanged) ложь
		this.settings = {//(91)Устанавливаем настройки(settings)
			margin: options.margin || 0//(92)Передаем в настройки margin из опцый(options) по умолчанию margin = 0
		}
		...
		this.changeCurrentSlide = this.changeCurrentSlide.bind(this);//
		this.changeAcniveDotClass = this.changeAcniveDotClass.bind(this);//(153)
		this.changeAcniveSlideClass = this.changeAcniveSlideClass.bind(this);//Актив для текущего слайда///////////////////////////////
	...
	manageHTML() {//(7) Делаем все необходимые объвертки
		this.containerNode.classList.add(GalleryClassName3);//(12) Добовляем в HTML GalleryClassName(class="gallery")
		//(13) Изменяем внутренний HTML елемента через innerHTML добовляем: GalleryLineClassName (обвертка для слайдов)
		//(101) Добовляем классы для стрелок и точек
		this.containerNode.innerHTML = `
	        <div class="${GalleryLineClassName3}">
	            ${this.containerNode.innerHTML}
	        </div>
	        <div class="${GalleryNavClassName3}">
	            <button class="${GalleryNavLeftClassName3}">Left</button>
	            <button class="${GalleryNavRightClassName3}">Right</button>
	        </div>
	        <div class="${GalleryDotsClassName3}"></div>
		`;//
		this.lineNode = this.containerNode.querySelector(`.${GalleryLineClassName3}`);//(14) Получаем линию в javaScript
		this.dotsNode = this.containerNode.querySelector(`.${GalleryDotsClassName3}`);//(108)Определяем this.dotsNode (родитель в которо находятся все точки)

        //(15)Определяем slideNodes
		this.slideNodes = Array.from(this.lineNode.children).map((childNode) =>//(16)Делаем масив из всех дочерних элементов (слайдов)
			wrapElementByDiv({//(17)Вызываем функцию обернуть элемент в див(функция будет вызывать новую обвернутую ноду)
				element: childNode,//(18)Передаем елементу дочернии элементы
				className: GallerySlideClassName3//(19)Передаем className переменную слайда	
			})
		);
        ...
        //(109)Изменяем внутренний HTML елемента через innerHTML(что бы добавить точки gallery-dots)
		this.dotsNode.innerHTML = Array.from(Array(this.size).keys()).map((key) => (//(109)Создаем масив который сформирует столько точек сколько слайдов
			`<button class="${GalleryDotClassName3} ${key === this.currentSlide ? 
			GalleryDotActiveClassName3 : '' }"></button>`//(110)Определяем кнопки которые будут отвечать за точки и добавляем активную
		)).join('');//(111)Получаем строку из кнопок методом присоединиться(join) по пустой строке

		this.dotsNodes = this.dotsNode.querySelectorAll(`.${GalleryDotClassName3}`);//(112)Получаем в javaScript
		this.navLeft = this.containerNode.querySelector(`.${GalleryNavLeftClassName3}`);//(113)Получаем в javaScript
		this.navRight = this.containerNode.querySelector(`.${GalleryNavRightClassName3}`);//(114)Получаем в javaScript
		this.changeAcniveSlideClass();//Актив для текущего слайда//////////////////////////////////////////////////////
	}
	...
	startDrag(evt) {//(54)
        ...
        //(55) Добавляем событие перемещение указателя (pointermove) в нее передаем событие this.dragging
		window.addEventListener('pointermove', this.dragging);//(55) в перетаскивание(this.dragging) будут производится все основные расчеты
		this.containerNode.classList.add(GalleryDraggableClassName3);//(97)Когда галерея тянится добавляем ей класс (gallery-draggable) 
	}

	stopDrag() {//(57)
		window.removeEventListener('pointermove', this.dragging);//(58)Удаляем событие перемещение указателя (pointermove)
		this.containerNode.classList.remove(GalleryDraggableClassName3);//(98)Когда галерея остановилась убираем класс (gallery-draggable) 
		this.changeCurrentSlide()//(131)
	}
	...
	dragging(evt) {//
		...
		// Chenge active slide
		if (//(68)Если
			dragShift > 5 &&//(69)перетаскиваемыйСдвиг(dragShift) больше 10 и 
			dragShift > 0 &&//(70)перетаскиваемыйСдвиг(dragShift) больше 0 и 
			!this.currentSlideWasChanged &&//(73)Не текущийСлайдБылИзменен(currentSlideWasChanged) нужен для того чтоб условие сработало один раз(по умолчанию folse ложный)
			this.currentSlide > 0//(71)текущийСлайд(currentSlide) больше 0
		) {
		...
		if (//(74)Если
			dragShift < -5 &&//(75)перетаскиваемыйСдвиг(dragShift) меньше -10 и 
			dragShift < 0 &&//(76)перетаскиваемыйСдвиг(dragShift) меньше 0 и
			!this.currentSlideWasChanged &&//(77)Не текущийСлайдБылИзменен(currentSlideWasChanged) нужен для того чтоб условие сработало один раз(по умолчанию folse ложный)
			this.currentSlide < this.size - 1//(78)текущийСлайд(currentSlide) меньше -1
		) {
	...
	changeAcniveDotClass() {//(152)Задаем метод сменаКлассаАктивнойТочке
		for(let i = 0; i < this.dotsNodes.length; i++) {//(154))Цыкл для(for) по цыклу проходим от 0 до количества точек
			this.dotsNodes[i].classList.remove(GalleryDotActiveClassName3);//(155)Удаляем у всех точек класс GalleryDotActiveClassName(gallery-dot-active)
		}
		this.dotsNodes[this.currentSlide].classList.add(GalleryDotActiveClassName3);//(156)Точке привязанной к текущему слайду добавляем класс GalleryDotActiveClassName(gallery-dot-active)
	}

    
    changeAcniveSlideClass() {//Актив для текущего слайда///////////////////////////////////////////
		for(let i = 0; i < this.slideNodes.length; i++) {////////////////////////////////////////////////////
			this.slideNodes[i].classList.remove(GallerySlideActiveClassName3);//////////////////////////////////////////////////////////////////////
		}
		this.slideNodes[this.currentSlide].classList.add(GallerySlideActiveClassName3);/////////////////////////////////////////////////
	}
...
new Gallery3(document.getElementById('gallery3'), {// создаем экземпляр Пallery, передаем обвертку gallery, вызываем через метод getElementById
    margin: 10// Отступ между слайдами
});
